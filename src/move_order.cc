...
constexpr std::array<MoveScore, 7> v{1, 3, 3, 5, 10, 0, 0};

MoveScore get_move_priority(const Move move, search::Thread &t,
                            const Square last_destination) {
  const MoveType move_type = GetMoveType(move);
  if (!IsQuiet(move)) {
    constexpr MoveScore base = 160;
    ...
    if (move_type == kCapture) {
      ...
      return (1 << 9) + base * v[piece_type] + capture_score;
    }
    if (move_type == kEnPassant) {
      int32_t capture_score = t.get_capture_score(piece, des, kPawn);
      return (1 << 9) + base + capture_score;
    }
    int32_t capture_score = t.get_capture_score(piece, des, piece_type);
    MoveScore promo_value = v[move_type - kKnightPromotion + kKnight];
    promo_value += v[piece_type];
    return (1 << 9) + base * promo_value + capture_score;
  }
...
struct MoveOrderInfo {
  ...
  MoveOrderInfo(const Board &board) :
...
MoveScore GetMoveWeight(const Move move, search::Thread &t, const MoveOrderInfo &info) {
  ...
  if (num_made_moves > 0 && info.last_move != kNullMove) {
    const Square last_destination = GetMoveDestination(info.last_move);
